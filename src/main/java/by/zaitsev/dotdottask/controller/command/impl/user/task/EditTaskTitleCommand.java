...
public class EditTaskTitleCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        if (optionalProject.isPresent()) {
            ...
            logger.log(Level.ERROR, "Cannot update task title, there are no project with id {}", projectId);
            throw new CommandException("Cannot update task title, there are no project with id " + projectId);
        }
        Task task;
        ...
        try {
            if (taskValidator.isTitleValid(title)) {
                TaskServiceImpl.getInstance().updateTaskTitleById(taskId, title);
                task.setTitle(title);
                logger.log(Level.DEBUG, "execute(HttpServletRequest request) method was completed " +
                        "successfully. Forwarded to catalog page");
            } else {
                request.setAttribute(AttributeName.UPDATE_TASK_TITLE_RESULT, ValidationResult.INVALID);
            }
