...
/**
 * DeleteAssignedUserCommand is used to delete assigned user.
 *
 * @author Konstantin Zaitsev
 */
public class DeleteAssignedUserCommand implements Command {
    private static final Logger logger = LogManager.getLogger(DeleteAssignedUserCommand.class);
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        long userId = Long.parseLong(request.getParameter(ParameterName.ASSIGNED_USER_ID));
        ...
        if (optionalProject.isPresent()) {
            ...
            logger.log(Level.ERROR, "Cannot delete user, there are no project with id {}", projectId);
            throw new CommandException("Cannot delete user, there are no project with id " + projectId);
        }
        try {
            Optional<User> optionalUser = UserServiceImpl.getInstance().findEntityById(userId);
            if (optionalUser.isPresent()) {
                UserServiceImpl.getInstance().deleteAssignedUserByProjectId(projectId, userId);
                project.getAssignedUserList().remove(optionalUser.get());
                logger.log(Level.DEBUG, "execute(HttpServletRequest request) method was completed " +
                        "successfully. Forwarded to catalog page");
            } else {
                request.setAttribute(AttributeName.DELETE_ASSIGNED_USER_RESULT, ValidationResult.INVALID);
            }
        } catch (ServiceException e) {
            logger.log(Level.ERROR, "User cannot be deleted: {}", e.getMessage());
            throw new CommandException("User cannot be deleted: ", e);
        }
