...
/**
 * EditTaskIsDoneCommand is used to edit task is done status.
 *
 * @author Konstantin Zaitsev
 */
public class EditTaskIsDoneCommand implements Command {
    private static final Logger logger = LogManager.getLogger(EditTaskIsDoneCommand.class);
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        String isDone = request.getParameter(ParameterName.IS_DONE);
        ...
        if (optionalProject.isPresent()) {
            ...
            logger.log(Level.ERROR, "Cannot update task is done status, there are no project with id {}",
                    projectId);
            throw new CommandException("Cannot update task is done status, there are no project with id " + projectId);
        }
        Task task;
        if (optionalTask.isPresent()) {
            task = optionalTask.get();
        } else {
            logger.log(Level.ERROR, "Cannot update task is done status, there are no task with id {}", taskId);
            throw new CommandException("Cannot update task is done status, there are no task with id " + taskId);
        }
        var taskValidator = TaskValidator.getInstance();
        try {
            if (taskValidator.isIsDoneValid(isDone)) {
                boolean newIsDone = Boolean.parseBoolean(isDone);
                TaskServiceImpl.getInstance().updateTaskIsDoneById(taskId, newIsDone);
                ...
                request.setAttribute(AttributeName.UPDATE_TASK_IS_DONE_RESULT, ValidationResult.INVALID);
            }
